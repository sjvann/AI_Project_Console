...
仲介是我們營運的公開 SaaS。需求公司發布專案；工程師找案；達成協議後抽仲介費。加入免費，認證要強。成交後才開 [公司工作區](modules/projects-clients.md)（現有 Company.* 改租戶，不重做）。做工與工時走 [控制台](../user/getting-started.md)。

## 要解決的問題

有軟體需求的公司不一定養得起正職團隊，也不一定會寫 PRD。獨立工程師找得到案子，成交後卻回到試算表、私人 Git、口頭保密。缺的是：可驗證的雙方、寫得清楚的需求、合同、以及成交後真的能開工的工具鏈。
...
| 主體 | 誰 | 能做（通過認證後） |
|------|-----|-------------------|
| 需求組織 | 任何有軟體需求的公司或單位，不限軟體業 | 用網頁精靈發布專案、邀請或接受應徵、簽署合同、進入工作區 |
...
1. 為該需求組織開啟或掛上**租戶工作區**（建議：一需求公司一個工作區、多專案；見 D-13）。
2. 把成交工程師寫進名冊（綁 GitHub），控制台申報目的地指向該工作區接收 API。
3. 為該案建立或掛上 GitHub 倉。**Issue 是唯一任務溝通面**；不另做站內聊天當主路徑。
4. 需求公司在工作區做管理面（專案彙整、工時確認、預算／戰情）。工程師繼續用控制台。

工作區不重做進件表、編譯、Agent。那些是控制台現況。
...
| 網頁精靈（成交前） | 只讀進件彙總（成交後有倉才有） | 需求工作台（成交後） |
| 不碰原始碼 | 不碰原始碼 | 原始碼只在工程師電腦與 GitHub |

禁止：仲介層複製一套薪資／戰情室；控制台連仲介或工作區資料庫；把控制台改成網站好讓需求公司發案。
...
一組通過認證的需求組織與工程師：發布 → 應徵或邀請 → 雙方接受 → 合同紀錄產生 → 開工作區且名冊有該工程師 → 控制台握手並申報第一筆工時。原始碼未上傳到仲介雲。
