...
**規劃**（成交後工作區：彙總、規則、核准、匯出）。工程師繪製執行狀態與本機計時為 **現況** 控制台能力，本模組消費上傳結果。仲介抽成應收不在本模組。
...
1. **現況：** 開啟專案即計時；儀表板可看週／月與各專案時數；可匯出 CSV。
2. **規劃：** 在控制台畫**執行狀態圖**（哪段日期投入哪個專案、完成度或階段備註），與時段一併送到**選定的公司接收 API**。可設定多家目的地；一次只送一家。
3. 上傳前先握手，確認該公司名冊有此工程師。上傳是明確動作（按「送到〔這家公司〕」），不是背景偷傳。失敗要能重試。本機仍保留 `work-hours.json`。公司端收到後依專案彙整，進入 PM 確認。
