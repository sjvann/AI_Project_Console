...
- 成功：時段入「待 PM 確認」；回傳伺服器 `timesheetId` 與狀態。公司工作區導覽「公開回報」是接收匣（Base URL、待確認、待歸戶、金鑰）；設定 → 公開回報可複製本實例 URL。
- 409：已核准時段不可覆蓋。
- 名冊／專案解析失敗：人話錯誤碼。
...
| 從工作區讀回 | 工時儀表板「公司」分頁：握手身分、該目的地派工（標本機能否對上）、最近上傳結果、可選薪資條 | 自行呼叫同一 GET |
| 堆疊／編譯／Agent | 有 | 無（不需要） |

控制台不嵌入工作區網頁、不複製甘特或戰情室。未設定申報目的地時，控制台行為維持現況：本機匯出，不上傳；工時「公司」分頁說明去設定 → 申報公司。
