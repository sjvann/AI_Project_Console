...
**現況**指桌面控制台已出貨行為。**規劃**指仲介平台與成交後工作區，除非該列已標現況。

## 產品邊界

```mermaid
flowchart LR
  subgraph market [仲介平台_規劃]
    Members[會員與認證]
    Listings[需求精靈發布]
    Deal[成交合同抽成]
  end
  subgraph console [控制台_現況]
    Stack[堆疊與編譯]
    Gh[GitHub_PR_Issue]
    Hours[本機工時]
    Intake[需求工作台]
  end
  subgraph workspace [公司工作區_成交後]
    People[人員與外包]
    Dispatch[派工]
    Projects[專案與客戶]
    Budget[預算]
...
控制台**不**改成網站。工程師繼續用本機視窗。仲介與工作區是瀏覽器。需求公司發案用仲介網頁精靈，不用控制台。

## 三層產品

控制台、公司工作區、仲介平台是**三個獨立產品**，不是同一套系統的前後台。**衝突時以本節為準。** 舊稱「雙產品切割」併入本節。

| | 控制台 | 公司工作區 | 仲介平台 |
|---|--------|------------|----------|
| 給誰 | 任何工程師（分析、設計、開發、測試） | 成交後的需求公司：經營／交付／人資／財務 | 需求組織與工程師（成交前） |
| 何時能用 | 不必先有仲介或工作區；本機堆疊、GitHub、工時、進件自成閉環 | 成交後由仲介開租戶；頂多規定「參與本專案的工程師須用控制台申報」 | 加入免費；通過認證才能發布／應徵／成交 |
| 面對幾家 | **一對多**：可設定多家工作區的接收 API | **一需求公司一租戶**（建議），名冊只認自己的人 | 我們營運的公開多租戶 SaaS |
| 部署 | 工程師本機安裝包（現況） | 預設我們主機上的租戶；自架改後期 | 我們營運 |

申報時數／績效的閉環（成交後）：

1. 工程師在控制台新增**申報目的地**（該工作區接收 API 的 HTTPS 位址；可多筆）。
...
禁止：控制台連仲介或工作區資料庫；工作區把控制台當內嵌元件發行；仲介層重做薪資／戰情室；把原始碼收到我們的雲。
...
| **成交** | 需求方與工程師雙方接受協議；產出合同套件並開工作區 |
| **仲介費** | 成交時平台應收；第一刀記帳，金流託管下一刀 |
| **租戶工作區** | 成交後為該需求組織開的公司管理實例。沿用 Company.*，不是第二套產品 |
| **客戶** | 工作區裡付款或需求的外部（或內部）甲方。仲介成交的需求組織，在工作區裡通常就是這個租戶自己 |
...
仲介層（成交前）另有 `demand_admin`（需求組織窗口）、`talent`（工程師會員）。細節見 [仲介平台](marketplace.md)。下表是**成交後工作區**角色（沿用）。
...
工作區**讀這些真相**，不複製一套本機堆疊 UI。仲介需求精靈不取代控制台進件；成交後有倉才落到 Git。
