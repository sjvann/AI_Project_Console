...
3. **申報目的地（可零筆）：** 顯示名、Base URL、契約版本。按「測試連線」→ `GET /api/v1/me`。通過才允許「送到〔這家公司〕」。失敗：「這家公司還沒有你的人員檔，請找對方人資」或「這個網址沒有公開回報契約」，不要 `403 Forbidden`。
...
全域導覽（權限內才出現）：戰情室、專案、客戶、派工、人員、外包商、預算、**公開回報**、薪資、帳戶、設定。設定內含：公司、公開回報、主題、匯率。人員帳戶在「帳戶」。工程師只見「我的工時／薪資條」與說明「繪圖請用控制台」。公開回報是接收匣：Base URL、待 PM 確認、待歸戶、API 金鑰；頁首徽章顯示待處理筆數。
...
列：人員，外包商可摺疊。欄：週一到五或週彙總。拖放派工（MVP 若時程緊可先用對話框建立，拖放列 V1.1）。超載格子紅底（`--color-danger`）。側欄：未派 Issue。
...
上：分頁「薪資週期｜待 PM 確認」。週期用頁籤一次只展開一張表。加班金額、鎖定原因在對話框，必填原因。不要把所有歷史週期的表疊成一條捲軸。控制台送來的時段先在導覽「公開回報」出現；薪資頁的待確認是同一批資料。

### 公開回報接收匣（pm／delivery／hr／owner）

上：本實例 Base URL（可複製給工程師）與契約端點。中：分頁「待 PM 確認｜待歸戶｜API 金鑰」。下：確認歸屬／退回。空狀態說明「工程師在控制台按送到〔這家公司〕後會出現」。不要把接收口藏在薪資週期裡當唯一入口。
...
在工時儀表板加一頁「狀態圖」：專案列 × 日期，塗色表示投入；存本機。設定裡可新增多個**申報目的地**（公司名＋接收 API）。送出時選一家，按鈕「送到〔這家公司〕」。不要做成公司平台甘特的縮小版。目的地必須已測試通過。
