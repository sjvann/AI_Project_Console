...
3. **申報目的地（可零筆、可多家）：** 設定 → **申報公司**。清單每一張卡片是一家公司；「再加一家公司」加入第二家、第三家。同一工程師可對不同公司的不同專案申報。按「測試連線」→ `GET /api/v1/me`。通過才允許「送到〔這家公司〕」。失敗：「這家公司還沒有你的人員檔，請找對方人資」或「這個網址沒有公開回報契約」，不要 `403 Forbidden`。
