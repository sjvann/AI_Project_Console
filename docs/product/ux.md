...
| 進件落到 Git、開 GitHub Issue 與驗收 | 桌面需求工作台（現況）；工作區只讀彙總 | 真相在 Git 倉 |
...
現況使用者已會選專案目錄。規劃精靈**不打斷**零目的地的現況路徑。服務列與專案列名稱旁顯示一句用途（清單 `description`／`projects`、專案檔 `Description`、套件 `description`）。沒有該欄就不填，也不讀 README，避免開啟很多專案時把文件整份讀完而停住。開啟或重掃工作區時寫入 `.ai_project/product-purposes.md`。
...
2. 「建置」選單、列上「編譯」、啟動／停止／開啟：忙碌時**事前**停用，`title`＝「忙碌中：建置 3/12 · Foo」或列上的「開啟中…／啟動中…」。不要等點了才彈原生「請等待」。服務列按「開啟」時，在佔用條出現前就標「開啟中…」並鎖按鈕；開啟結束（成功、失敗、取消）由該動作清掉標記，不要靠下一次健康檢查才消失。
