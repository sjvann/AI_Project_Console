...
- **Rule:** 編譯輸出合併刷新（約 100–200ms 一批）。可見 log 用環形緩衝（建議最後 2 000 行）。佔用期間降低或暫停 `PollLoop` 的 Git／健康／新鮮度全掃。健康檢查必須**平行**探測各服務（離線 HTTP 各有超時）；禁止串行 20 幾個超時把輪詢卡住，否則服務列過渡字與就緒數會假死。取消必須在重繪風暴下仍收得到。
