...
| 讀者 | 工程、設計、產品、採購（會後走讀） |
| 權威來源 | [願景](vision.md)、[總規格](spec.md)、[公開回報契約](reporting.md)、[系統分析輔助](analysis.md)、[路線圖](roadmap.md)、[系統執行計劃書](execution-plan.md) |
...
| 產品 | 本 PRD 波次 | 給誰 | 營運 |
|------|-------------|------|------|
| 控制台 | 現況＋A 的上傳閉環 | 任何軟體工程師 | 本機安裝包 |
| 公司工作區 | A | 要管人／專案／毛利的公司 | 預設我們主機租戶；自架後期 |
| 系統分析輔助 | B | PM／分析／有概念的窗口 | 我們營運 Web |
| 仲介平台 | C（門檻後） | 非軟體業廠商、工程師會員 | 我們營運的公開 SaaS |
...
一家公司開帳工作區；工程師（控制台或其他符合契約的用戶端）握手並上傳第一筆回報；依專案看得到貢獻；人員／派工／工時確認／薪資 CSV／毛利／戰情室可營運（沿用 Company.* 租戶化）。

技術形態（見 [架構](architecture.md)）：工作區是我們營運的 .NET 10 **Blazor Web App**；控制台維持 Photino.Blazor，**不改成網站**。公開契約見 [reporting.md](reporting.md)。
...
規格：各 [modules](modules/people.md)、[reporting.md](reporting.md)。以下原波次 B 條目升為波次 A；**PRD-CON-\*** 與公開契約進 A。
...
| PRD-PLT-04 | A | `owner` | 工作區預設是我們主機上的一個租戶 | 自架改後期。控制台可對多家租戶申報 |
...
| ID | 主題 | 要求 |
|----|------|------|
| PRD-NFR-01 | 部署 | 工作區預設由我們營運；TLS。自架後期。仲介同（C） |
...
**波次 A 通過**當且僅當 KPI-01～08、PRD-PLT／PPL／PRJ／DSP／PAY／BDG／WAR／CON／RPT 為真（沿用程式處以租戶可營運為準）。
