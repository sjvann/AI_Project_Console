...
GitHub 入口是 **Pulse 狀態列**（專案名稱那一列底下的綠底列）與 **操作台**（點標題列右側「GitHub 操作台」或帳號燈後展開四欄與任務清單）。開發與需求兩個工作台都能用。多數日常動作（看狀態、提交）不需先填設定；同步、發布、Release、PR 需要知道 owner／repo。

Pulse 只顯示**一個**下一步（未提交 → 提交；落後 → 同步；超前／無追蹤 → 發布；功能分支尚無 PR → 建立 PR；CI 進行中或剛失敗 → Actions；預設分支已與遠端一致 → 發行 Release）。其餘動作按「日常／協作／發行／接入」分組，在 Hub 裡。快捷鍵 **Ctrl+G** 開／關操作台。

檢查此控制台有沒有新版，請用設定裡的「檢查更新」，不在 GitHub 操作台裡。

## 登入（GitHub 管理的專案必填）

開啟的目錄若已接上遠端 Git 主機（預設 `github.com`，或設定／remote 上的企業主機），控制台會要求用 [GitHub CLI](https://cli.github.com/) 對**該主機**登入。未登入不能繼續使用該專案。

1. 安裝 `gh` 後，按對話框「登入」。可改認證主機（例如 `ghe.corp.com`）。控制台會開啟瀏覽器，並顯示一次性代碼（可複製）。請把代碼貼到 GitHub 授權頁。
2. 登入後，標題列帳號燈顯示 `@帳號`（綠＝就緒、黃＝有異動待處理；點一下開操作台）；非公開主機時加上主機名稱。未登入為紅燈「未登入」。登出在操作台頂欄。
3. GitHub 操作台下方列出指派給你的 Issue；點「我的任務」標題會先打開內容（含圖）。先討論：不是問題就結案；自己做得到就開分支、建 PR；處理不了才請 Agent。未指派清單可開合，也能「接受」成自己的任務。
...
| **提交…** | 列出未提交檔（可勾選要進本次提交的、預覽 diff），填標題與本次交付說明後只 `git add` 已勾選檔並 commit。可勾選提交後 push。說明可按「AI 建議」（依已勾選檔案；有 [Cursor Agent CLI](https://cursor.com/docs/cli/overview) 時用 Agent，否則依 diff 起草） |
...
關閉專案、切換專案、按「離開」，只要工作區**不乾淨**或還有**未發布提交**（含尚未追蹤遠端的新分支），會跳出警示。請先提交並發布，讓本機與遠端一致。若確定本機狀態可丟，警示視窗有**強行關閉**（切換專案時為**強行切換專案**）。
...
| **開啟倉庫** | 瀏覽器開倉庫首頁 |
| **建立／開啟 PR** | 已有 PR 則看檢查摘要（能不能請人審）。沒有則用提交說明建立。必須已在功能分支上提交並發布；從預設分支（如 `main`）開會失敗。從「我的任務」視窗建立時會在說明寫入 `Closes #編號`；自己或 Agent 做完再建立。討論後若不是問題請改按「結案」。行內討論仍開遠端網頁；需求工作台可請人審、並在 CI 綠後合併 |
