...
GitHub 入口是 **Pulse 狀態列**（專案名稱那一列底下的綠底列）與 **操作台**（點「GitHub 操作台」後在工具列正下方展開四欄與任務清單；開發台的按鈕在右側、專案問答與環境體檢左邊）。開發與需求兩個工作台都能用。多數日常動作（看狀態、提交）不需先填設定；同步、發布、Release、PR 需要知道 owner／repo。
