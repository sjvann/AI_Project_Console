...
3. GitHub 操作台下方列出指派給你的 Issue；點「我的任務」標題會在控制台開啟視窗。**送出回應**是留言回報；**建立 PR** 是用目前分支交作業（合併後關閉 Issue）。兩者不是同一件事。未指派清單可開合，也能「接受」成自己的任務。
...
| **開啟倉庫** | 瀏覽器開倉庫首頁 |
| **建立／開啟 PR** | 已有 PR 則看檢查摘要（能不能請人審）。沒有則用提交說明建立。必須已在功能分支上提交並發布；從預設分支（如 `main`）開會失敗。從「我的任務」視窗建立時會在說明寫入 `Closes #編號`。行內討論仍開遠端網頁；需求工作台可請人審、並在 CI 綠後合併 |
