...
2. GitHub → Actions → **Pack Unix** → Run workflow。
   - `version`：目前版號（例如 `0.6.19`，不含 `v`）
   - `upload_release`：若 `v0.6.19` 已經存在，勾選後會把 Unix 資產附加到該 Release（不覆寫 `*-win-x64-setup.exe`）
3. 工作流程會：
...
```powershell
gh workflow run pack-unix.yml -f version=0.6.19 -f upload_release=true
gh run watch
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/pack-unix.ps1 -Version 0.6.19 -Runtime osx-arm64
```
