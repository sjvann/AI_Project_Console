...
目前程式版號見 `src/AiProject.Console.Core/AppInfo.cs` 的 `Version`（例如 `0.3.4`）。下列指令以 `0.3.5` 為例，請改成實際新版號。
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/pack-win.ps1 -Version 0.3.5
```

成功後 `dist\` 會有：

- `AI_Project_Console-0.3.5-win-x64-setup.exe`：安裝程式（開始選單捷徑，可選桌面捷徑）
- `AI_Project_Console-0.3.5-win-x64.zip`：免安裝壓縮包，解壓後執行 `AI_Project_Console.exe`
...
```powershell
gh release create v0.3.5 --title "v0.3.5 AI_Project 控制台" --notes @"
## AI_Project 控制台 0.3.5

<在此寫本版說明>

### 下載

- **AI_Project_Console-0.3.5-win-x64-setup.exe**：安裝程式（開始選單捷徑，可選桌面捷徑；安裝到目前使用者的 Local AppData）
- **AI_Project_Console-0.3.5-win-x64.zip**：免安裝壓縮包，解壓後執行 ``AI_Project_Console.exe``

Windows 10/11 需已安裝 [WebView2](https://developer.microsoft.com/microsoft-edge/webview2/)（系統通常已內建）。
"@ dist\AI_Project_Console-0.3.5-win-x64-setup.exe dist\AI_Project_Console-0.3.5-win-x64.zip
```

完成後網址：

`https://github.com/sjvann/AI_Project_Console/releases/tag/v0.3.5`
